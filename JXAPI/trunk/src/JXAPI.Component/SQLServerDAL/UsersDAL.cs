            ...
            try
            {
                ...
                switch (tableName.ToLower())
                {
                    case "users":
                        ...
                        sqlCommand.Append(string.Format("  FROM {0} WHERE UID={1} ORDER BY UID", tableName, id));
                        break;
                    case "usersbase":
                    ...
                    case "usersreceiver":
                        ...
                        sqlCommand.Append(string.Format("  FROM {0} WHERE UID={1} ORDER BY ReceiverID", tableName, id));
                        break;
                    case "usersaccount":
                        sqlCommand.Append(string.Format("  Ident,UID,Deposit,RemainingSum,Creator,CreateTime,Remarks,Status,UpdateTime,ISNULL(OrderNO,0) as OrderNO FROM {0} WHERE UID={1} ORDER BY Ident", tableName, id));
                        break;
                    case "usersscore":
                        sqlCommand.Append(string.Format("  ScoreID,UID,Score,ScoreType,GetMethod,CreateTime,ISNULL(OrderNO,0) as OrderNO FROM {0} WHERE UID={1} ORDER BY ScoreID", tableName, id));
                        break;
                }
