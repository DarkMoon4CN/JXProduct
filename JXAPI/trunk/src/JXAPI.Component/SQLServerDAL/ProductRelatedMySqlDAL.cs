...
namespace JXAPI.Component.SQLServerDAL
{
    public class ProductRelatedMySqlDAL
    {
        ...
        /// <param name="errorCount"></param>
        /// <returns></returns>
        public bool UpdateProductRelatedEx(DataTable productTable, out int errorCount)
        {
            var flag = true;
            errorCount = 0;
            ...
            for (int i = 0; i < productTable.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (result <= 0)
                    {
                        errorCount++;
                        flag = false;
                        myLog.ErrorFormat("UpdateProductRelated 更新商品关联信息表失败,商品关联信息ID：{0},受影响行为0", dr["RelatedID"]);
                    }
                }
                catch (Exception ex)
                {
                    myLog.ErrorFormat("UpdateProductRelated 更新商品关联信息表失败,商品关联信息ID：{0},异常信息:{1}", dr["RelatedID"], ex.Message);
                    flag = false;
                    errorCount++;
                }
            }
