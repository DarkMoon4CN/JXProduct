...
namespace JXAPI.Component.SQLServerDAL
{
    public class ClassificationMySqlDAL
    {
        ...
        public bool UpdateClassificationEx(DataTable table, out int errorCount)
        {
            var flag = true;
            errorCount = 0;
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                {
                    StringBuilder sqlCommand = new StringBuilder();
                    sqlCommand.Append("update classification set ");
                    var Placeholder = string.Format(@"ChineseName = '{0}',PinyinName = '{1}',ImagesLogo = '{2}',Level = '{3}',Path = '{4}',ParentID = '{5}',
                                                PathCount = '{6}',Tittle = '{7}',Keywords = '{8}',Description = '{9}',Sort = '{10}',Status = '{11}',RootCFID = '{12}',IsTop = '{13}',IsChannelHot = '{14}',keywordID = '{15}',ProductCount = '{16}'",
                                     dr["ChineseName"].ToString().Replace("\'", "\""), dr["PinyinName"].ToString().Replace("\'", "\"")
                                     , dr["ImagesLogo"].ToString().Replace("\'", "\""), dr["Level"].ToInt(), dr["Path"].ToString().Replace("\'", "\"")
                                     , dr["ParentID"].ToInt(), dr["PathCount"].ToShort(), dr["Title"].ToString().Replace("\'", "\""), dr["Keywords"].ToString().Replace("\'", "\"")
                                     , dr["Description"].ToString().Replace("\'", "\""), dr["Sort"].ToInt(), dr["Status"].ToShort(), dr["Path"].ToString().Substring(0, dr["Path"].ToString().IndexOf('/'))
                                     , dr["IsTop"].ToShort(), dr["IsChannelHot"].ToShort(), dr["keywordID"].ToInt(), dr["ProductCount"].ToInt());
                    sqlCommand.Append(Placeholder);
                    sqlCommand.AppendFormat(@" where CFID = {0}", dr["CFID"]);
                    ...
                    if (result <= 0)
                    {
                        errorCount++;
                        flag = false;
                        myLog.ErrorFormat("UpdateClassification 更新分类信息表失败,分类信息ID：{0},受影响行为0", dr["CFID"]);
                    }
                }
                catch (Exception ex)
                {
                    myLog.ErrorFormat("UpdateClassification 更新分类信息表失败,分类信息ID：{0},异常信息:{1}", dr["CFID"], ex.Message);
                    flag = false;
                }
            }
