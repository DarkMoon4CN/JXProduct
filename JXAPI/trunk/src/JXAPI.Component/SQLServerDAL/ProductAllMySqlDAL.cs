...
namespace JXAPI.Component.SQLServerDAL
{
    public class ProductAllMySqlDAL
    {
        ...
        public bool UpdateProductAllEx(DataTable table, out int errorCount)
        {
            var flag = true;
            errorCount = 0;
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                {
                    StringBuilder sqlCommand = new StringBuilder();
                    sqlCommand.Append("update ProductAll set ");
                    var Placeholder = string.Format(@"Favor = '{0}',Promotion = '{1}', Salled = '{2}',Creator = '{3}',Sales = '{4}',CreateTime = '{5}',Updater = '{6}',UpdateTime = '{7}'",
                                         dr["Like"].ToString().Replace("\'", "\""), dr["Promotion"].ToString().Replace("\'", "\""), dr["Salled"].ToString().Replace("\'", "\"")
                                        , dr["Creator"].ToString().Replace("\'", "\""), dr["Sales"].ToString().Replace("\'", "\""), dr["CreateTime"].ToDateTime().ToString(), dr["Updater"].ToString().Replace("\'", "\"")
                                        , dr["UpdateTime"].ToDateTime().ToString());
                    sqlCommand.Append(Placeholder);
                    sqlCommand.AppendFormat(@" where AllID = {0}", dr["AllID"]);
                    ...
                    if (result <= 0)
                    {
                        errorCount++;
                        flag = false;
                        myLog.ErrorFormat("UpdateProductAll 更新全站推荐表失败,全站推荐ID：{0},受影响行为0", dr["AllID"]);
                    }
                }
                catch (Exception ex)
                {
                    myLog.ErrorFormat("UpdateProductAll 更新全站推荐表失败,全站推荐ID：{0},异常信息:{1}", dr["AllID"], ex.Message);
                    flag = false;
                    errorCount++;
                }
            }
        ...
        public bool AddProductAll(DataTable table, out int errorCount)
        {
            ...
            {
                myLog.ErrorFormat("AddProductAll 添加全站推荐失败,全站推荐ID：{0}-{1},异常信息:{2}", table.Rows[0]["AllID"], table.Rows[table.Rows.Count - 1]["AllID"], ex.Message);
                flag = false;
            }
            return flag;
        }
