            ...
            try
            {
                ...
                switch (tableName.ToLower())
                {
                    case "coupon":
                        ...
                        sqlCommand.Append(string.Format(" AND ID > {0}  AND  CreateTime>'2015-08-20' ORDER BY ID", ID));
                    ...
                    case "coupontype":
                        sqlCommand.Append(string.Format("SELECT TOP {0} * FROM {1} WHERE 1=1 ", pageSize, tableName));
                        sqlCommand.Append(string.Format(" AND TypeID > {0} AND  CreateTime>'2015-08-20' ORDER BY TypeID", ID));
                        dbType_Add = DatabaseType.dbr_CouponBase;
                        break;
