...
namespace JXAPI.Component.SQLServerDAL
{
    public class ActivityMySqlDAL
    {
        ...
        public bool UpdateActivityEx(DataTable table, out int errorCount)
        {
            var flag = true;
            errorCount = 0;
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                {
                    StringBuilder sqlCommand = new StringBuilder();
                    sqlCommand.Append("update Activity set ");
                    var Placeholder = string.Format(@"BriefName = '{0}',Name = '{1}', Description = '{2}',URL = '{3}',Type = '{4}',Restrictive = '{5}',CountLimit = '{6}',UserLimit = '{7}',StartTime = '{8}',EndTime = '{9}',
                                                Updater = '{10}',UpdateTime = '{11}',Status = '{12}',UserType = '{13}'",
                                    dr["BriefName"].ToString().Replace("\'", "\""), dr["Name"].ToString().Replace("\'", "\""), dr["Description"].ToString().Replace("\'", "\"")
                                    , dr["URL"].ToString().Replace("\'", "\""), dr["Type"].ToShort(), dr["Limit"].ToShort(), dr["CountLimit"].ToInt()
                                    , dr["UserLimit"].ToInt(), dr["StartTime"].ToDateTime().ToString(), dr["EndTime"].ToDateTime().ToString()
                                    , dr["Updater"].ToString().Replace("\'", "\""), dr["UpdateTime"].ToDateTime().ToString(), dr["Status"].ToShort()
                                    , dr["UserType"].ToShort());
                    sqlCommand.Append(Placeholder);
                    sqlCommand.AppendFormat(@" where ActID = {0}", dr["ActID"]);
                    ...
                    if (result <= 0)
                    {
                        errorCount++;
                        flag = false;
                        myLog.ErrorFormat("UpdateActivity 更新活动信息表失败,活动信息ID：{0},受影响行为0", dr["ActID"]);
                    }
                }
                catch (Exception ex)
                {
                    myLog.ErrorFormat("UpdateActivity 更新活动信息表失败,活动信息ID：{0},异常信息:{1}", dr["ActID"], ex.Message);
                    flag = false;
                    errorCount++;
                }
            }
        ...
        public bool AddActivity(DataTable table, out int errorCount)
        {
            ...
            {
                myLog.ErrorFormat("AddActivity 添加活动信息失败,活动信息ID：{0}-{1},异常信息:{2}", table.Rows[0]["ActID"], table.Rows[table.Rows.Count - 1]["ActID"], ex.Message);
                flag = false;
            }
            return flag;
        }
