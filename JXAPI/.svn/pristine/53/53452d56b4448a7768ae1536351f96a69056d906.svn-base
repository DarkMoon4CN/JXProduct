...
namespace JXAPI.Component.SQLServerDAL
{
    public class ProductComboMySqlDAL
    {
        ...
        /// <param name="errorCount"></param>
        /// <returns></returns>
        public bool UpdateProductComboEx(DataTable productTable, out int errorCount)
        {
            var flag = true;
            errorCount = 0;
            for (int i = 0; i < productTable.Rows.Count; i++)
            {
                ...
                {
                    StringBuilder sqlCommand = new StringBuilder();
                    sqlCommand.Append("update productcombo set ");
                    var Placeholder = string.Format(@"ProductID = '{0}',ComboProductID = '{1}',Name = '{2}', Price = '{3}',Quantity = '{4}',Creator = '{5}', CreateTime = '{6}'",
                                      dr["ProductID"].ToInt(), dr["ComboProductID"].ToInt(), dr["Name"].ToString().Replace("\'", "\""), dr["Price"].ToDecimal()
                                     , dr["Quantity"].ToInt(), dr["Creator"].ToString().Replace("\'", "\""), dr["CreateTime"].ToDateTime().ToString());
                    ...
                    if (result <= 0)
                    {
                        errorCount++;
                        flag = false;
                        myLog.ErrorFormat("UpdateProductCombo 更新商品赠品表失败,商品赠品ID：{0},受影响行为0", dr["ID"]);
                    }
                }
                catch (Exception ex)
                {
                    myLog.ErrorFormat("UpdateProductCombo 更新商品赠品表失败,商品赠品ID：{0},异常信息:{1}", dr["ID"], ex.Message);
                    flag = false;
                }
            }
