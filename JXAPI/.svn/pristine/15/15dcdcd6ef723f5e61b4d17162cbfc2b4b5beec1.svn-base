...
namespace JXAPI.Component.SQLServerDAL
{
    public class ProductClassificationMySqlDAL
    {
        ...
        public bool UpdateProductClassification(DataTable productTable, out int errorCount)
        {
            ...
            {
                myLog.ErrorFormat("UpdateProductClassification 更新商品分类表失败,商品分类ID：{0}-{1},异常信息:{2}", productTable.Rows[0]["ProductID"], productTable.Rows[productTable.Rows.Count - 1]["ProductID"], ex.Message);
                flag = false;
            }
            return flag;
        }
        ...
        /// <param name="errorCount"></param>
        /// <returns></returns>
        public bool UpdateProductClassificationEx(DataTable productTable, out int errorCount)
        {
            var flag = true;
            errorCount = 0;
            for (int i = 0; i < productTable.Rows.Count; i++)
            {
                ...
                {
                    StringBuilder sqlCommand = new StringBuilder();
                    sqlCommand.Append("update productclassification set ");
                    var Placeholder = string.Format(@"ProductID = '{0}',CFID = '{1}',CFPath = '{2}'",
                                      dr["ProductID"].ToInt(), dr["CFID"].ToInt(), dr["CFPath"].ToString().Replace("\'", "\""));
                    ...
                    if (result <= 0)
                    {
                        errorCount++;
                        flag = false;
                        myLog.ErrorFormat("UpdateProductClassification 更新商品分类表失败,商品分类ID:{0},受影响行为0", dr["ID"]);
                    }
                }
                catch (Exception ex)
                {
                    myLog.ErrorFormat("UpdateProductClassification 更新商品分类表失败,商品分类ID：{0},异常信息:{1}", dr["ID"], ex.Message);
                    flag = false;
                    errorCount++;
                }
            }
