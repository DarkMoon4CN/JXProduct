...
namespace JXAPI.JXSdk.Service
{
    ...
    public class HeadLinesService
    {
        ...
        private static string updateSort = "headLines/updateSort";
        ...
        /// <param name="changUser">操作人</param>
        /// <returns></returns>
        public bool UpdateSort(int headID, int sort, int categoryID, string changUser)
        {
            HeadLinesRequest request = new HeadLinesRequest()
            {
                headID = headID,
            ...
            string postData = JsonHelper.GetJson(request);
            HeadLinesResponse response = client.Execute(request, updateSort, postData);
            if (response != null)
                return response.success;
            return false;
        }
