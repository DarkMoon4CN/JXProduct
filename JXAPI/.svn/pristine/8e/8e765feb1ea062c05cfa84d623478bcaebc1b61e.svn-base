...
namespace JXAPI.Component.SQLServerDAL
{
    public class ProductActivityMySqlDAL
    {
        ...
        /// <param name="errorCount"></param>
        /// <returns></returns>
        public bool UpdateProductActivityEx(DataTable productTable, out int errorCount)
        {
            var flag = true;
            errorCount = 0;
            for (int i = 0; i < productTable.Rows.Count; i++)
            {
                ...
                {
                    StringBuilder sqlCommand = new StringBuilder();
                    sqlCommand.Append("update productactivity set ");
                    var Placeholder = string.Format(@"ActID = '{0}',ActName = '{1}', ActStock = '{2}',ActQuantity = '{3}',ActPrice = '{4}',
                                                    ActDesc = '{5}',Type = '{6}',TypeLimit = '{7}',CouponBatchNo = '{8}',CouponName = '{9}',Discount = '{10}',ProductGiftID = '{11}',
                                                    IsFreeShip = '{12}',ExtType = '{13}',Status = '{14}',StartDate = '{15}',EndDate = '{16}',Creator = '{17}',CreateTime = '{18}',Updater = '{19}',UpdateTime = '{20}',
                                                    ProductCode = '{21}',MarketPrice = '{22}',TradePrice = '{23}',MobilePrice = '{24}',CostPrice = '{25}'",
                                    dr["ActID"].ToInt(), dr["ActName"].ToString().Replace("\'", "\""), dr["ActStock"].ToInt()
                                    , dr["ActQuantity"].ToInt(), dr["ActPrice"].ToDecimal(), dr["ActDesc"].ToString().Replace("\'", "\""), dr["Type"].ToShort(), dr["TypeLimit"].ToShort(), dr["CouponBatchNo"].ToString().Replace("\'", "\"")
                                    , dr["CouponName"].ToString().Replace("\'", "\""), dr["Discount"].ToDecimal(), dr["ProductGiftID"].ToInt(), dr["IsFreeShip"].ToShort()
                                    , dr["ExtType"].ToString().Replace("\'", "\""), dr["Status"].ToShort(), dr["StartDate"].ToDateTime().ToString(), dr["EndDate"].ToDateTime().ToString(), dr["Creator"].ToString().Replace("\'", "\"")
                                    , dr["CreateTime"].ToDateTime().ToString(), dr["Updater"].ToString().Replace("\'", "\""), dr["UpdateTime"].ToDateTime().ToString()
                    ...
                    if (result <= 0)
                    {
                        errorCount++;
                        flag = false;
                        myLog.ErrorFormat("UpdateProductActivity 更新单品活动表失败,单品活动ID{0},受影响行为0", dr["ProductID"]);
                    }
                }
                catch (Exception ex)
                {
                    myLog.ErrorFormat("UpdateProductActivity 更新单品活动表失败,单品活动ID：{0},异常信息:{1}", dr["ProductID"], ex.Message);
                    flag = false;
                    errorCount++;
                }
            }
