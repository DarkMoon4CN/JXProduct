            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(strPlaceholder))
                {
                    ...
                    if (result <= 0)
                    {
                        ...
                        flag = false;
                        myLog.ErrorFormat("AddOrderOperateLogHistory 添加订单操作日志失败,订单操作日志ID：{0}-{1},失败数:{2}", productTable.Rows[0]["LogID"], productTable.Rows[productTable.Rows.Count - 1]["LogID"], errorCount);
                    }
                    else
                    {
