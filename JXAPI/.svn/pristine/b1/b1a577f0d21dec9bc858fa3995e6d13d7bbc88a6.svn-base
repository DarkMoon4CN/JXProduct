...
{
    public class JXOrderMySqlDAL
    {
        private static Database dbw_Order = JXOrderMySqlData.Writer;
        ...
        public int GetMaxID(string tableName, string primaryId)
        {
            ...
            try
            {
                if (tableName == "JXOrderProduct")
                {
                    tableName = "Product";
                }
                else if (tableName == "JXOrderProductrelated")
                {
                    tableName = "productrelated";
                }
                string sqlCommand = string.Format(@"select * from {0} order by {1} DESC limit 0, 1", tableName, primaryId);
                var cmd = dbr_Order.GetSqlStringCommand(sqlCommand);
                if (dbr_Order.ExecuteScalar(cmd).IsNotNULL())
                {
                    maxId = Convert.ToInt32(dbr_Order.ExecuteScalar(cmd));
                }
                else
                {
                    maxId = 0;
        ...
        public OperationResult<bool> Update(string tableName, string[] colName, string[] sqlServerColName, DataRow dr)
        {
            try
            {
                ...
                var cmd = dbw_Order.GetSqlStringCommand(sqlCommand.ToString());
                ...
                var result = dbw_Order.ExecuteNonQuery(cmd);
        ...
        public OperationResult<bool> Add(string tableName, string[] colName, string[] sqlServerColName, DataRow dr)
        {
            try
            {
                ...
                var cmdExist = dbr_Order.GetSqlStringCommand(sqlExistCommand.ToString());
                if (Convert.ToInt32(dbr_Order.ExecuteScalar(cmdExist)) > 0)
                {
                    return new OperationResult<bool>(OperationResultType.Success, null, true);
                }
