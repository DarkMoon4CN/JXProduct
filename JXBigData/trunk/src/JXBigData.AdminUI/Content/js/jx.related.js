...
related.dabaozhuang = function () {
    ...
    $formdbz.on("click", ".table a[name=edit]", function () {
        ...
        $quan.val($tds.eq(4).find("span").text());
        $price.val($tds.eq(3).find("span").text());
...
related.zuhe = function () {
    ...
    var defaulttr = '<tr><td>{0}</td><td>{1}</td><td class="tcName">{2}</td><td>{3}</td><td class="yeMian">{4}</td><td>{5}</td><td>{6}</td><td class="cz"><a href="#" name="edit">编辑</a><a href="#" name="del">删除</a></td></tr>'.format(related.productid, related.productcode, related.chinesename, related.spec, related.chinesename, related.price, 1);
    var $zuhe = $("#formzuhe");
        ...
        $.post("/product/ComboDetail", { productid: productid }, function (result) {
            if (result.status) {
                ...
                for (i = 0; i < result.data.length; i++) {
                    ...
                    html.push('<tr><td>' + item.ComboProductID + '</td><td>' + item.ComboProductCode + '</td><td class="tcName">' + item.ComboProductName + '</td><td>' + item.Spec + '</td><td>' + item.Name + '</td><td>' + item.Price + '</td><td quantity="' + item.Quantity + '">' + item.Quantity + '</td><td><a href="#" name="edit">编辑</a>' + (item.IsMain ? "" : '<a href="#" name="delrow">删除</a>') + '</td></tr>');
                }
    ...
    function setViewTemp() {
        ...
        var html = '<tr><td>' + $childid.val() + '</td><td>' + childcode + '</td><td class="tcName">' + childname + '</td><td>' + childspec + '</td><td class="yeMian">' + $viewname.val() + '</td><td>' + price + '</td><td quantity="' + count + '">' + count + '</td><td><a href="#" name="edit">编辑</a><a href="#" name="del">删除</a></td></tr>';
