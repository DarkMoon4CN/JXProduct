...
product.price = function () {
    ...
    $('#packlist li').on("click", function () {
        ...
        product.get(productid, function (data) {
            var ischeckprice = data.IsCheckPrice == 1;
            $productid.text(data.ProductID);
            $productode.text(data.ProductCode);
            $chinseeName.html((ischeckprice ? "<span style='color:red'>[限价]</span>" : "") + data.ChineseName + (data.Specifications.length > 0 ? " [" + data.Specifications + "]" : ""));
            $marketprice.val(data.MarketPrice);
            $costprice.text(data.CostPrice);
            $tradeprice.val(data.TradePrice).prop("readonly", true);
            $mobileprice.val(data.MobilePrice).prop("readonly", true);
        ...
        if (isedit) {
            ...
            $.post("/product/checkproductcode", { productcode: productcode }, function (result) {
                if (!result.status) {
                    ...
                    var str = '<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td><td>{8}</td><td>{9}</td><td index="' + index + '"><a href="#" name="edit">编辑</a><a href="#" name="del">删除</a><input type="hidden" name="newproduct{10}" value="{11}"></td></tr>';
...
productedit.upload = function () {
    ...
    $('.addImg-btn').on('click', function () {
        ...
        if (desccount > 20)
            return false;
        var html = '<div class="operAdverW clear"><div class="InformaUpload clear"><span class="fl"><i class="xingRed">*</i>详情图{0}：</span><div class="resetUpBtn fl"><span id="descimage{1}"><img src="/content/images/upImg.png" alt=""></span></div><span class="imgInfo fl">大小：1024PX*1024PX</span></div><div class="informa-imgWrap hide" id="descimagequeue{1}"></div></div>';
...
productadd.operate = function () {
    ...
    $form.validate({
        ...
        ignore: ":hidden:not('#BrandID,#SupplierID')",
        errorPlacement: function (error, element) { error.insertAfter(element); },
        submitHandler: function () {
            if ($(".ptype :checked").length > 0) {
            ...
            $.ajax({
                type: "POST", url: "/product/addproduct", data: $form.serialize(), dataType: "json", success: function (result) {
                    if (result.status) {
                        ...
                        $table.find("tbody").empty().html("<tr><td colspan='10' align=center>无</td></tr>")
                        $hidden_count.val(0);
                        count = 0;
                        productcodes = [];
...
productadd.submitSuccess = function (data) {
    ...
    var temp = '<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td><td>{8}</td></tr>';
    var str = [];
    for (var i = 0; i < data.length; i++) {
        str.push(temp.format(i + 1, data[i].ProductID, data[i].ProductCode, data[i].TradeName, data[i].Specifications, data[i].CADN, data[i].Manufacter, data[i].SupplierName));
    }
