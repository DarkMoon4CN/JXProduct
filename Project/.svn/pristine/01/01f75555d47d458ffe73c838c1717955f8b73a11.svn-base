...
namespace JXProduct.Component.SQLServerDAL
{
    internal class ProductActivityDAL
    {
        ...
        public bool ProductActivity_Update(ProductActivityInfo model)
        {
            ...
            dbw.AddInParameter(dbCommand, "TypeLimit", DbType.Int16, model.TypeLimit);
        ...
        internal ProductActivityInfo ProductActivity_Get(int productID)
        {
            ...
            using (var dataReader = dbr.ExecuteReader(dbCommand))
            {
                if (dataReader.Read())
                {
        ...
        private ProductActivityInfo RecoverModel(IDataReader dataReader)
        {
            ...
            //ProductID	ActID	ActName	ActStock	ActQuantity	ActPrice	ActDesc	Type	TypeLimit	CouponBatchNo	
            //CouponName	Discount	ProductGiftID	IsFreeShip	ExtType	Status	StartDate	EndDate	Creator	CreateTime	Updater	UpdateTime
            ...
            if (DBNull.Value != dataReader["ActivityType"])
                model.ActivityType = Convert.ToInt32(dataReader["ActivityType"]);
            if (DBNull.Value != dataReader["ActivityName"])
