...
K.options = {
	...
	items: [
		...
		'insertunorderedlist', '|', 'emoticons', 'image', 'link', 'unlink', 'clearhtml', '|', 'fullscreen'],
...
_plugin('core', function(K) {
	...
	var selForecolor = '', selHilitecolor = '';
	_each('forecolor,hilitecolor'.split(','), function (i, name) {
	    self.clickToolbar(name, function () {
            //  2015-07-21  记忆上一次选择颜色 LCJ
	        var col = (name == "hilitecolor") ? selHilitecolor : selForecolor;
	        if (col.length > 0) {
	            self.exec(name, col).hideMenu();
	            return;
	        }
	        self.createMenu({
				name : name,
